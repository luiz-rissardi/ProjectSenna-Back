...
export class UpdateGroup extends UseCase {
    ...
    async execute(chatId, groupName, groupDescription, groupPhoto) {
        try {
            ...
            if (group.isValid()) {
                const result = await this.repository.patchOne(group);
                if (result.isSuccess) {
    ...
    async #groupExists(groupId) {
        const result = await this.repository.findOne(groupId)
...
const useCase = new UpdateGroup(repositoryContext);
